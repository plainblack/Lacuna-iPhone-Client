...
@implementation SendSpiesController
...
- (void)viewDidLoad {
	...
	self.navigationItem.title = @"Send Spies";
	...
	self.sectionHeaders = _array([LEViewSectionTab tableView:self.tableView withText:@"From"], [LEViewSectionTab tableView:self.tableView withText:@"Ship"], [LEViewSectionTab tableView:self.tableView withText:@"Spies"]);
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (self.sendFromBodyId && !self.spyShips) {
...
- (void)prepareDataLoaded:(LEBuildingPrepareSendSpies *)request {
...
- (void)spiesLeft:(LEBuildingSendSpies *)request {
	if (![request wasError]) {
		[self.navigationController popViewControllerAnimated:YES];
	}
}
...
+ (SendSpiesController *)create {
	return [[[SendSpiesController alloc] init] autorelease];
}
