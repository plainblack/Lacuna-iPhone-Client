...
+ (LETableViewCellParagraph *)getCellForTableView:(UITableView *)tableView {
    ...
    if (cell == nil) {
		cell = [[LETableViewCellParagraph alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
...
+ (CGFloat)getHeightForTableView:(UITableView *)tableView text:(NSString *)text {
	CGRect frame = CGRectMake(0, 0, tableView.bounds.size.width-10, tableView.bounds.size.height);
